...
import { LocalAI } from './hilum-ai.js';

describe('LocalAI.preload', () => {
  let cacheDir: string;
  ...
  it('returns cached path when model is already in cache', async () => {
    ...
    const result = await LocalAI.preload('user/repo/model.gguf', { cacheDir });
  ...
  it('calls onProgress callback', async () => {
    ...
    const result = await LocalAI.preload('user/repo/model.gguf', { cacheDir, onProgress });
  ...
  it('throws on invalid model specifier', async () => {
    await expect(
      LocalAI.preload('invalid-specifier', { cacheDir }),

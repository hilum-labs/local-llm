...
import { extractModelFilename, resolveModelUrl, type CacheEntry } from 'local-llm-js-core';
...
export class ModelManager {
  ...
  async downloadModel(url: string, options?: DownloadOptions): Promise<string> {
    ...
    const filename = extractModelFilename(resolvedUrl);
...
async function downloadToFile(
  ...
  if (totalSize > 0) {
    ...
    if (fileStat.size !== totalSize) {
      throw new Error(`Download incomplete: expected ${totalSize} bytes, got ${fileStat.size}`);
    }
  }
}
